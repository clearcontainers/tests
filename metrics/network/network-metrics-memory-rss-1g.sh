...
#
# Description:
# This metrics test measures the Resident Set Size memory while an interconnection
# between container-client <----> container-server transfers 1 Gb rate as a
# network workload using nuttcp.
#
# The selection of 1 Gb as a tranfer rate is due to that is
...
TEST_NAME="network metrics memory rss 1g"
...
function rss_memory() {
	...
	echo "RSS memory: $memory Kb"

	save_results "$TEST_NAME" "rss 1g" "$memory" "kb"
