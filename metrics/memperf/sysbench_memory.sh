...
#   This test runs the 'sysbench memory' test in a container, and extracts
#   the transfer bandwidth result.
...
function main()
{
	...
	declare -a ir
	local ir=($(fgrep transferred <<< "$output" | awk '{printf "%s %s", $4, $5}' | sed 's/[()]//g'))
	local result=${ir[0]}
	local units=${ir[1]}
